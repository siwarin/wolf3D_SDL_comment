/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   wolf.h                                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: siwarin <marvin@42.fr>                     +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/08/29 14:59:41 by siwarin           #+#    #+#             */
/*   Updated: 2019/10/18 10:41:50 by cduverge         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#ifndef WOLF_H
# define WOLF_H

# include "../Libft/libft.h"
# include "SDL2/SDL.h"
# include <math.h>
# include <stdio.h>
# define WIDTH	1300
# define HEIGTH	850

typedef struct		s_keyboard
{
	const Uint8			*state;
}									t_keyboard;

typedef struct		s_fps
{
	double	step;
	double	current;
	double	delta;
	double	last;
	double	count;
}					t_fps;

typedef struct		s_tex
{
	void			*img;
	uint32_t		*data;
	int				bpp;
	int				s_l;
	int				endian;
}					t_tex;

typedef struct		s_pars
{
	char			*str;
	int				len;
	struct s_pars	*next;
}					t_pars;

typedef struct		s_player
{
	double			x;
	double			y;
	double			dirx;
	double			diry;
	double			camx;
	double			rayposx;
	double			rayposy;
	double			raydirx;
	double			raydiry;
	double			speed;
	double			rot;
}					t_player;

typedef struct		s_sdl
{
	SDL_Window			*win;
	SDL_Surface			*ren;
	SDL_Event				e;
	t_keyboard			k;
	/*void			*img;
	int				*adr;
	int				bpp;
	int				s_l;
	int				endian;*/
	int				m;
	int				fd;
	int				**map;
	int				x_max;
	int				y_max;
	int				cy;
	int				cx;
	double			planex;
	double			planey;
	int				mapx;
	int				mapy;
	double			sdistx;
	double			sdisty;
	double			ddistx;
	double			ddisty;
	double			pwalldist;
	int				hit;
	int				stepx;
	int				stepy;
	int				side;
	int				hl;
	int				ds;
	int				de;
	int		color;
	t_player		play;
	t_tex			tex[9];
	t_fps			f;
	int				i;
	int				j;
	int				t;
	int				o;
	int				mu;
	int				md;
	int				ml;
	int				mr;
	int				rr;
	int				rl;
	int				textx;
	int				texty;
	int				id;
	double			wallx;
	double			fx;
	double			fy;
	double			dw;
	double			dp;
	double			cd;
	double			w;
	double			cfx;
	double			cfy;
	int				ftx;
	int				fty;
	int				cbp;
	int				ft;
}					t_sdl;

int					hook_keydown(t_sdl *sdl);
void				ft_error(t_sdl *sdl, int n);
void				draw(t_sdl *sdl);
int					check_line(char *l);
int					check_and_fill(t_pars *pars, t_pars *cur, char *line);
int					check_first_line(int fd, t_pars **pars);
int					check_file(t_pars *pars, int fd);
int					parsing(char *av, char *av1, char *av2, t_sdl *sdl);
int					parsing_3(t_sdl *sdl);
void				fix_map(t_sdl *sdl);
int					check_wall(t_sdl *sdl);
int					malloc_pars(t_pars *begin);
int					**ft_tabnew_two(size_t x, size_t y);
void				free_pars(t_pars *pars);
void				pixel(t_sdl *sdl, int x, int y);
void				clear_image(t_sdl *sdl);
void				del_image(t_sdl *sdl);
int					keyboard(t_sdl *sdl);
void				wolfinit(t_sdl *sdl);
int					keyrelease(t_sdl *sdl);
void				turn(t_sdl *sdl);
int					move(t_sdl *sdl);
void				draw_wall(t_sdl *sdl);
void				draw_color(t_sdl *sdl);
void				fisheye(t_sdl *sdl);
void				turn2(t_sdl *sdl);
void				load_t(t_sdl *sdl);
void				keyboard2(t_sdl *sdl);
void				draw_sky(t_sdl *sdl);
void				next_floor(t_sdl *sdl);
void				options(t_sdl *sdl);
void				minimap(t_sdl *sdl);
int					ft_is_less_than_seven(int c);
int					check_coordinates(char *x, char *y, t_sdl *sdl);
int					am_i_in_a_wall(t_sdl *sdl);

/*
** colors
*/

# define BLACK 0x000000
# define WHITE 0xFFFFFF
# define RED 0xFF0000
# define ORANGE 0xFFA500
# define LIME 0x00FF00
# define BLUE 0x0000FF
# define YELLOW 0xFFFF00
# define CYAN 0x00FFFF
# define MAGENTA 0xFF00FF
# define SILVER 0xC0C0C0
# define GRAY 0x808080
# define MAROON 0x800000
# define OLIVE 0x808000
# define GREEN 0x008000
# define PURPLE 0x800080
# define TEAL 0x008080
# define NAVY 0x000080

/*
** More awesome colors!
*/

# define CRIMSON 0xDC143C
# define CORAL 0xFF7F50
# define INDIAN_RED 0xCD5C5C
# define SALMON 0xFA8072
# define ORANGE_RED 0xFF4500

# define GOLD 0xFFD700
# define GOLDEN_ROD 0xDAA520
# define DARK_GOLDEN_ROD 0xB8860B
# define SADDLEBROWN 0x8B4513

# define LAWN_GREEN 0x7CFC00
# define DARK_GREEN 0x006400
# define FOREST_GREEN 0x228B22
# define PALE_GREEN 0x98FB98
# define SPRING_GREEN 0x00FF7F
# define SEA_GREEN 0x2E8B57
# define LIGHT_SEA_GREEN 0x20B2AA

# define DARK_SLATE_GRAY 0x2F4F4F
# define POWDER_BLUE 0xB0E0E6

# define AQUA_MARINE 0x7FFFD4
# define STEEL_BLUE 0x4682B4
# define SKY_BLUE 0x87CEEB
# define MIDNIGHT_BLUE 0x191970

# define INDIGO 0x4B0082
# define DARK_MAGENTA 0x8B008B
# define DEEP_PINK 0xFF1493
# define HOT_PINK 0xFF69B4
# define ORCHID 0xFF83FA

# define BEIGE 0xF5F5DC
# define ANTIQUE_WHITE 0xFAEBD7
# define WHEAT 0xF5DEB3
# define CORN_SILK 0xFFF8DC

# define LAVENDER 0xE6E6FA
# define FLORAL_WHITE 0xFFFAF0
# define ALICE_BLUE 0xF0F8FF
# define GHOST_WHITE 0xF8F8FF
# define HONEYDEW 0xF0FFF0
# define IVORY 0xFFFFF0
# define AZURE 0xF0FFFF
# define SNOW 0xFFFAFA
# define MISTY_ROSE 0xFFE4E1

# define DARK_GRAY 0x696969
# define GAINSBORO 0xDCDCDC
# define WHITE_SMOKE 0xF5F5F5

#endif
